          ...
          <ul class="list-inline">
              ...
              <li class="list-inline-item">
                  <a id="asign-tag-button">
                    ...
                    {{__("file-manager::messages.asing_tag")}}
                  </a>
              </li> 
              <li class="list-inline-item">
...
@push('after_scripts')
        ...
        document.querySelectorAll(`${prefix}.selected-media`).forEach(element => {
            ...
            element.addEventListener('click', e => {
                ...
                modal.querySelector('.modal-save').addEventListener('click', e => {
                    const mediaData = {
                        parent : modal.querySelector('select[name="parentId"]').value,
    ...
    const renderMediasTable = (medias, prefix, type) => {
        ...
        // initBulkOperations(prefix);
    }

    const getMedias = (page = 1, tags=null, type = false, callback = false) => {
        // tags = tags !== '' ? `/${tags.join('-')}` : '';
        // type = type ? type : 0;
        apiRequest(`/admin/media/fetch/media`, mediasResponse => {
    ...
    const initCropper = (i) => {
        ...
        buttonConfirm.addEventListener('click', function (e) {
            ...
            canvas.toBlob(function (blob) {
                ...
                const croppedImage = new File([blob], "crop_image.jpeg", {
                    type: blob.type,
                });
    ...
    document.addEventListener('DOMContentLoaded', () => {
        console.log('Running fm init')
        const fileManagerContainer = document.querySelector('.custom-file-manager');
        console.log('COntainer:', fileManagerContainer);
